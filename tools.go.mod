// Tool-only module: pins versions for the dev/CI tools go-bzlmod-ast
// invokes via `go tool -modfile=tools.go.mod ...`. Kept out of the main go.mod
// so the runtime dep graph stays clean — golangci-lint's transitive
// closure is ~250 modules and has no business in `go list -deps`
// output for downstream consumers.
//
// Matches the portfolio standard split (per `memory/feedback_go_project_conventions.md`).
module github.com/albertocavalcante/go-bzlmod-ast/tools
